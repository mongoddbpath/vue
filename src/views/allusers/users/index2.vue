...
export default {
  ...
  data () {
    return {
      ...
      pagination: {
        background: true,
        currentPage: parseInt(localStorage.getItem('usercontrolcurrentPage')) || 1,
        pageSize: 15,
        total: 0
      },
      columns: [
  ...
  mounted () {
    let getcurrentPage = parseInt(localStorage.getItem('usercontrolcurrentPage')) || 1
  ...
  methods: {
    ...
    paginationCurrentChange (currentPage) {
      ...
      localStorage.setItem('usercontrolcurrentPage', currentPage)
      this.fetchData()
    },
    // 前往创建站点
    gotowebs () {
