...
export default {
  ...
  data () {
    return {
      ...
      pagination: {
        background: true,
        currentPage: parseInt(localStorage.getItem('rechargeczcurrentPage')) || 1,
        pageSize: 15,
        total: 12
      },
      columns: [
  ...
  mounted () {
    ...
    let getcurrentPage = parseInt(localStorage.getItem('rechargeczcurrentPage')) || 1
  ...
  methods: {
    ...
    fetchData () {
      this.loading = true
      rechargelist({ page: this.pagination.currentPage, type: 1 }).then(res => {
    ...
    paginationCurrentChange (currentPage) {
      ...
      localStorage.setItem('rechargeczcurrentPage', currentPage)
      this.fetchData()
    },
    // 前往创建站点
    gotowebs () {
